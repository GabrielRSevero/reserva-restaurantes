    ...
    <Box className="bg-white/10 rounded-md">
      <ol className="items-center w-full sm:flex">
        {steps.map((step: IStep, key: number) => (
          <li 
            key={key} 
            className={`flex items-center text-white space-x-2.5 rtl:space-x-reverse px-6 py-4 cursor-pointer ${currentStep === key + 1 ? "bg-gradient-to-r from-violet-600 to-indigo-600 rounded-tl-md" : "hover:bg-white/20"}`} 
